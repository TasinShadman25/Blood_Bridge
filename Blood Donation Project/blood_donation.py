...
# ================= HOME =================
if menu == "🏠 Home":
    st.title("🩸 Blood Donation Dashboard - Home")
...
# --- COLUMN 1: Buttons + Recent Requests ---
    with col1:
        ...
        if not requests_df.empty:
            st.dataframe(
                requests_df[['patient_name', 'required_blood_group', 'location', 'hospital_name']].tail(5),
    ...
    # ===============================
    # 💡 COLUMN 2: Map + Blood Group Requests
    # ===============================
    with col2:
        st.subheader("📍 Donor Locations Map")

        # Only keep donors in Bangladesh
        # Only keep donors in Bangladesh
        donors_df_bangladesh = donors_df.copy()  # Keep all donors; assume all are in Bangladesh

        # Add latitude and longitude if missing
        if 'latitude' not in donors_df_bangladesh.columns or 'longitude' not in donors_df_bangladesh.columns:
            from geopy.geocoders import Nominatim
            from geopy.extra.rate_limiter import RateLimiter

            geolocator = Nominatim(user_agent="blood_donation_app", timeout=10)  # increase timeout
            geocode = RateLimiter(geolocator.geocode, min_delay_seconds=1)

            donors_df_bangladesh['full_location'] = donors_df_bangladesh['location'] + ', Bangladesh'
            ...
            donors_df_bangladesh[['latitude','longitude']] = donors_df_bangladesh['full_location'].apply(lambda x: pd.Series(safe_geocode(x)))
            
            # Save coordinates in CSV so we don't geocode every run
            donors_df_bangladesh.to_csv(DONORS_CSV, index=False)

        # Drop rows without coordinates
        map_df = donors_df_bangladesh.dropna(subset=['latitude','longitude'])

        # Show map
        if not map_df.empty:
            st.map(map_df[['latitude','longitude']])
        else:
            st.info("No donor location data available.")
    ...
    if st.button("Search Donors"):
        ...
        if df.empty:
            ...
            if filtered.empty:
                ...
                st.dataframe(filtered[['name','blood_group','phone','location','eligible_status']])
                
    if st.button("Search Closest Donor & Hospital"):
        closest_donor = find_closest_donor(loc or "Dhaka", bg or "A+")
        closest_hospital = find_closest_hospital(loc or "Dhaka")
        if closest_donor is not None:
            st.subheader("Closest Donor:")
            st.write({
        ...
        else: st.warning("No matching donor found.")
        ...
        else: st.warning("No nearby hospital found.")
...
# ================= REQUEST BLOOD (Add / Remove) =================
    ...
    with st.form("request_form", clear_on_submit=True):
        ...
        phone = st.text_input("Phone")
        location = st.text_input("Location")
        required_blood_group = st.selectbox("Required Blood Group", ["A+","A-","B+","B-","AB+","AB-","O+","O-"])
        emergency = st.checkbox("Emergency")
        hospital_name = st.text_input("Hospital Name")
        submit_request = st.form_submit_button("Submit Request")
        if submit_request:
            requester = Requester(patient_name, phone, location, required_blood_group, emergency, hospital_name)
            save_request(requester)
...
# ================= REQUEST BLOOD LIST =================
    ...
    if REQUESTS_CSV.exists():
        ...
        if df.empty:
            ...
            st.dataframe(
                filtered[['patient_name','required_blood_group','phone','location','emergency','hospital_name']],
                use_container_width=True
            )
    else:
